...
def compare_protocols(image_path, triton_grpc_url="localhost:8001", triton_http_url="http://localhost:8000", num_requests=20):
    ...
    print("Testing gRPC performance...")
    grpc_results = benchmark_performance(image_path, triton_grpc_url, num_requests)
    
    # Test HTTP performance (if available)
    ...
    try:
        ...
        def send_http_request(image_bytes, triton_url):
            payload = {
                "inputs": [
                    {
                        "name": "INPUT_IMAGE", 
        ...
        # Warm up
        send_http_request(image_bytes, triton_http_url)
        ...
        for i in range(num_requests):
            ...
            try:
                response = send_http_request(image_bytes, triton_http_url)
                end_time = time.time()
        ...
        if times:
            http_avg_time = np.mean(times) * 1000
            http_throughput = 1000 / http_avg_time
            
            print(f"   HTTP Average latency: {http_avg_time:.2f} ms")
            print(f"   HTTP Throughput: {http_throughput:.2f} requests/second")
            
            # Compare results
            if grpc_results:
                improvement = ((http_avg_time - grpc_results['avg_time_ms']) / http_avg_time) * 100
                throughput_improvement = ((grpc_results['throughput_rps'] - http_throughput) / http_throughput) * 100
                
                print(f"   gRPC is {improvement:.1f}% faster in latency")
                print(f"   gRPC has {throughput_improvement:.1f}% better throughput")
                
                if improvement > 20:
                    print("gRPC shows significant performance improvement!")
                elif improvement > 0:
                    print(" gRPC shows moderate performance improvement")
                else:
                    print("Performance difference is minimal")
        
    except ImportError:
        print("requests module not available for HTTP comparison")
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        ...
        help="Triton gRPC server URL (default: localhost:8001)"
    )
    parser.add_argument(
        "--http-url",
        type=str, 
    ...
    parser.add_argument(
        "--compare",
        action="store_true",
        help="Compare gRPC vs HTTP performance"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if not test_server_health(args.grpc_url):
    ...
    if args.image:
        ...
        # Single image test
        success = test_single_image(image_path, args.grpc_url, args.verbose)
        if not success:
            return 1
        
        # Run benchmark
        if args.benchmark:
            benchmark_performance(image_path, args.grpc_url, args.benchmark)
        
        # Compare protocols if requested
        if args.compare:
            compare_protocols(image_path, args.grpc_url, args.http_url, args.benchmark)
